...
import random
import base64
import requests # Back to standard, stable requests
...
# Create a session to keep cookies and connection alive (mimics a browser tab)
...
def send_message_logic(channel_id, text, img_path):
    ...
    try:
        ...
        if img_path and os.path.exists(img_path):
            # Standard requests handles multipart/form-data perfectly with the 'files' parameter
...
def main():
    ...
    while True:
        for cid in CHANNEL_IDS:
            ...
            print(f"{'✅' if success else '❌'} {cid}")
            time.sleep(2) # 2s delay to prevent Discord global rate limit
        
        # No long breaks, just a quick loop reset
        time.sleep(1)
